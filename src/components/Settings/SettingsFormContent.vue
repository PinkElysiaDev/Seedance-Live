  ...
  <div>
    ...
    <div v-if="props.tab === 'provider'" class="space-y-6">
      <div class="flex flex-wrap items-center gap-3 bg-ak-dark border border-gray-800 p-3">
        ...
        <button class="bg-ak-dark border border-gray-800 text-gray-400 hover:border-ak-400 hover:text-ak-400 font-mono text-xs px-4 py-2 transition-colors shadow-sm" @click="addSeedance">+ ADD_OFFICIAL</button>
        <button class="bg-ak-dark border border-gray-800 text-gray-400 hover:border-ak-400 hover:text-ak-400 font-mono text-xs px-4 py-2 transition-colors shadow-sm" @click="addCustom">+ ADD_CUSTOM</button>
        <button class="bg-red-950/20 border border-red-500/30 text-red-500 hover:bg-red-500 hover:text-ak-darker font-mono text-xs px-4 py-2 transition-colors shadow-sm" @click="removeCurrent">PURGE</button>
      </div>
      ...
      <template v-if="profile">
        ...
        <div class="flex flex-wrap items-center gap-4 pt-6 border-t border-gray-800 mt-6">
          ...
          <span class="text-xs font-mono text-gray-500 ml-auto border border-gray-800 bg-ak-darker px-3 py-1">{{ profile.kind === 'seedance' ? '// OFFICIAL VOLCENGINE MODE' : '// CUSTOM TEMPLATE MODE' }}</span>
        </div>
        ...
        <div v-if="profile.kind === 'custom' && profile.custom" class="mt-8 space-y-8 border-l-2 border-ak-400/50 bg-ak-darker/30 p-6 shadow-[inset_0_0_20px_rgba(0,0,0,0.5)]">
          <div class="space-y-6">
            <div class="flex items-center gap-3 border-b border-ak-400/20 pb-2">
               ...
               <div class="text-sm font-sans italic font-bold text-ak-400 tracking-widest">SUBMIT_CFG //</div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
              <label class="block md:col-span-2 flex flex-col gap-2">
                <span class="text-xs font-mono text-ak-400 block tracking-wider">SUBMIT_URL</span>
              ...
              <label class="block flex flex-col gap-2">
                <span class="text-xs font-mono text-ak-400 block tracking-wider">HTTP_METHOD</span>
            ...
            <div class="bg-ak-dark/50 p-4 border-l border-gray-800">
              <div class="flex items-center justify-between mb-4">
                <span class="text-xs font-mono text-gray-500">HEADERS_PAYLOAD //</span>
                <button class="text-xs font-mono text-ak-400 hover:text-white flex items-center gap-1" @click="addSubmitHeader"><span class="text-lg leading-none">+</span> ADD_HEADER</button>
              </div>
              <div class="space-y-3">
                <div v-for="h in submitHeaders" :key="h.k" class="flex gap-4 items-end border-b border-dashed border-gray-800 pb-3">
                  ...
                  <button class="text-gray-500 hover:text-red-500 font-mono text-sm px-2 pb-0.5 transition-colors" title="Delete Header" @click="delSubmitHeader(h.k)">[-]</button>
                </div>
              </div>
            </div>
            <label class="block flex flex-col gap-2">
              <span class="text-xs font-mono text-ak-400 block tracking-wider">BODY_TEMPLATE (JSON format) //</span>
          ...
          <div class="space-y-6 pt-6 border-t border-gray-800">
            <div class="flex items-center gap-3 border-b border-ak-400/20 pb-2">
               <div class="w-2 h-2 bg-ak-400 shadow-[0_0_8px_rgba(0,229,255,0.4)]"></div>
               <div class="text-sm font-sans italic font-bold text-ak-400 tracking-widest">POLL_CFG //</div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
              <label class="block md:col-span-2 flex flex-col gap-2">
                <span class="text-xs font-mono text-ak-400 block tracking-wider">POLL_URL</span>
            ...
            <div class="bg-ak-dark/50 p-4 border-l border-gray-800">
              <div class="flex items-center justify-between mb-4">
                <span class="text-xs font-mono text-gray-500">HEADERS_PAYLOAD //</span>
                <button class="text-xs font-mono text-ak-400 hover:text-white flex items-center gap-1" @click="addPollHeader"><span class="text-lg leading-none">+</span> ADD_HEADER</button>
              </div>
              <div class="space-y-3">
                <div v-for="h in pollHeaders" :key="h.k" class="flex gap-4 items-end border-b border-dashed border-gray-800 pb-3">
                  ...
                  <button class="text-gray-500 hover:text-red-500 font-mono text-sm px-2 pb-0.5 transition-colors" title="Delete Header" @click="delPollHeader(h.k)">[-]</button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="space-y-6 pt-6 border-t border-gray-800">
             <div class="flex items-center gap-3 border-b border-ak-400/20 pb-2">
               <div class="w-2 h-2 bg-ak-400 shadow-[0_0_8px_rgba(0,229,255,0.4)]"></div>
               <div class="text-sm font-sans italic font-bold text-ak-400 tracking-widest">RESPONSE_EXTRACTION_PATHS //</div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <label class="flex flex-col gap-2"><span class="text-[10px] font-mono text-gray-500 block tracking-wider">TASK_ID_PATH</span><input :value="profile.custom.taskIdPath" class="w-full bg-ak-dark border-b border-gray-800 text-white font-mono text-sm py-1.5 focus:border-ak-400 outline-none transition-colors" @input="patchCustom({ taskIdPath: ($event.target as HTMLInputElement).value })" /></label>
              <label class="flex flex-col gap-2"><span class="text-[10px] font-mono text-gray-500 block tracking-wider">STATUS_PATH</span><input :value="profile.custom.statusPath" class="w-full bg-ak-dark border-b border-gray-800 text-white font-mono text-sm py-1.5 focus:border-ak-400 outline-none transition-colors" @input="patchCustom({ statusPath: ($event.target as HTMLInputElement).value })" /></label>
              <label class="flex flex-col gap-2"><span class="text-[10px] font-mono text-gray-500 block tracking-wider">SUCCESS_VALUES (CSV)</span><input :value="profile.custom.successValues.join(',')" class="w-full bg-ak-dark border-b border-gray-800 text-white font-mono text-sm py-1.5 focus:border-ak-400 outline-none transition-colors" @input="patchCustom({ successValues: ($event.target as HTMLInputElement).value.split(',').map(s=>s.trim()).filter(Boolean) })" /></label>
              <label class="flex flex-col gap-2"><span class="text-[10px] font-mono text-gray-500 block tracking-wider">FAILURE_VALUES (CSV)</span><input :value="profile.custom.failureValues.join(',')" class="w-full bg-ak-dark border-b border-gray-800 text-white font-mono text-sm py-1.5 focus:border-ak-400 outline-none transition-colors" @input="patchCustom({ failureValues: ($event.target as HTMLInputElement).value.split(',').map(s=>s.trim()).filter(Boolean) })" /></label>
              <label class="flex flex-col gap-2"><span class="text-[10px] font-mono text-gray-500 block tracking-wider">VIDEO_URL_PATH</span><input :value="profile.custom.videoUrlPath" class="w-full bg-ak-dark border-b border-gray-800 text-white font-mono text-sm py-1.5 focus:border-ak-400 outline-none transition-colors" @input="patchCustom({ videoUrlPath: ($event.target as HTMLInputElement).value })" /></label>
              <label class="flex flex-col gap-2"><span class="text-[10px] font-mono text-gray-500 block tracking-wider">LAST_FRAME_URL_PATH</span><input :value="profile.custom.lastFrameUrlPath ?? ''" class="w-full bg-ak-dark border-b border-gray-800 text-white font-mono text-sm py-1.5 focus:border-ak-400 outline-none transition-colors" @input="patchCustom({ lastFrameUrlPath: ($event.target as HTMLInputElement).value })" /></label>
              <label class="flex flex-col gap-2"><span class="text-[10px] font-mono text-gray-500 block tracking-wider">ERROR_PATH</span><input :value="profile.custom.errorPath ?? ''" class="w-full bg-ak-dark border-b border-gray-800 text-white font-mono text-sm py-1.5 focus:border-ak-400 outline-none transition-colors" @input="patchCustom({ errorPath: ($event.target as HTMLInputElement).value })" /></label>
              <label class="flex flex-col gap-2"><span class="text-[10px] font-mono text-gray-500 block tracking-wider">PROGRESS_PATH</span><input :value="profile.custom.progressPath ?? ''" class="w-full bg-ak-dark border-b border-gray-800 text-white font-mono text-sm py-1.5 focus:border-ak-400 outline-none transition-colors" @input="patchCustom({ progressPath: ($event.target as HTMLInputElement).value })" /></label>
              <label class="flex flex-col gap-2"><span class="text-[10px] font-mono text-gray-500 block tracking-wider">POLL_INTERVAL_SEC</span><input type="number" :value="profile.custom.pollIntervalSec" class="w-full bg-ak-dark border-b border-gray-800 text-white font-mono text-sm py-1.5 focus:border-ak-400 outline-none transition-colors" @input="patchCustom({ pollIntervalSec: Number(($event.target as HTMLInputElement).value) })" /></label>
            </div>
    ...
    <div v-else-if="props.tab === 'proxy'" class="space-y-6 max-w-2xl">
      ...
      <div class="bg-ak-dark/30 p-4 border-l-2 border-gray-800">
        <p class="font-mono text-xs text-gray-400 leading-relaxed">
          <span class="text-ak-400 font-bold">// NOTICE:</span> 官方接口浏览器直连通常会被 CORS 拦截，需自备代理转发。代理脚本示例见 docs/proxy.md。
        </p>
      </div>
    ...
    <div v-else class="space-y-6 max-w-2xl">
      ...
      <div class="space-y-6 p-6 bg-ak-dark/50 border border-gray-800">
        <div class="flex flex-col gap-2">
          <span class="font-mono text-xs text-ak-400 tracking-wider">OFFICIAL_POLL_INTERVAL (SEC) //</span>
          <div class="flex items-center gap-2">
            ...
            <span class="font-mono text-xs text-gray-500">SECONDS_DELAY</span>
          </div>
        </div>
      </div>
